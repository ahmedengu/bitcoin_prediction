...
if __name__ == '__main__':
    ...
    bt_model_data = bt_market_info.drop('Date', 1)
    bt_model_data.columns = [str(col) + '_x' for col in bt_model_data.columns]

    # get market info for bitcoin from the start of 2016 to the current day
    bt_market_info = pd.read_html(
        "https://coinmarketcap.com/currencies/litecoin/historical-data/?start=" + start_date + "&end=" + end_date)[0]
    # convert the date string to the correct date format
    bt_market_info = bt_market_info.assign(Date=pd.to_datetime(bt_market_info['Date']))
    # look at the first few rows
    bt_market_info = bt_market_info.rename(columns={"Close**": "Close", "Open*": "Open"})
    ...
    model_data = pd.concat([bt_model_data, bt_market_info], axis=1)
    # ,'Volume','Market Cap'
    ...
    norm_cols = [v for i, v in enumerate(model_data.columns) if
                  not any(x in v for x in ['Date', 'bt_day_diff', 'bt_close_off_high', 'bt_volatility'])]
