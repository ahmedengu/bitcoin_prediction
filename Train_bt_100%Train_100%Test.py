...
if __name__ == '__main__':
    ...
    bt_market_info = pd.read_csv('BITFINEX_SPOT_BTC_USD_4HRS.csv')
    bt_market_info = bt_market_info.rename(
        columns={"price_close": "Close", "price_open": "Open", "price_high": "High", "price_low": "Low",
                 "time_period_start": "Date", "volume_traded": "Volume", "market_cap": "Market Cap"})
    bt_market_info = bt_market_info.drop(columns=["trades_count", "time_close", "time_open", "time_period_end"])
    ...
    training_set, test_set = model_data, model_data
    ...
    LSTM_test_inputs = LSTM_training_inputs  # ModelUtils.buildLstmInput(test_set, norm_cols, window_len)
    # model output is next price normalised to 10th previous closing price
    LSTM_test_outputs = LSTM_training_outputs  # ModelUtils.buildLstmOutput(test_set, 'bt_Close', window_len)
    ...
    book.save('Backtest_Output_100%Train_100%Test.xlsx')
